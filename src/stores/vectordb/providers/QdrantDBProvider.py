...
class QdrantDBProvider(VectorDBInterface):
    ...
    async def delete_collection(self, collection_name: str):
        if await self.is_collection_existed(collection_name=collection_name):
            self.logger.info(f"Deleting collection {collection_name} from Qdrant")
    ...
    async def create_collection(self, collection_name: str, embedding_size: int, do_reset: bool = False):
        ...
        if not await self.is_collection_existed(collection_name=collection_name):
            self.logger.info(f"Creating a new qdrant collection {collection_name} with embedding size {embedding_size} and distance method {self.distance_method}")
